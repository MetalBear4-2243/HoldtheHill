...
| 2026-09-23 | nilly-ctrl | Claude Code (Claude Opus 5) | Updated `CLAUDE.md` to describe the real branch model — `Indev` as the group integration/test branch, `main` as stale-and-deliberate — replacing the stale "working branch is `BryceTest`, PR into `main`" rule; also recorded the fork/upstream remotes and the noreply-email requirement | `Indev` is what we branch from and PR into until docs and structure move to `main`; updated README "How we work" to match | — docs only, no code touched |
| 2026-09-23 | MetalBear4-2243 | Gemini 3.8 Flash (High) | Implemented baseline `BasicEnemy` script with `double` health, `int` damage, path coordinate traversal, EditMode unit tests, and technical documentation | `BasicEnemy` component in `Features/Enemies/Scripts` with Transform-based movement, `EnemyPath` compatibility, and docs in `Documentation/EnemyScripts_Documentation.md` | Full scene Play mode with spawner |
