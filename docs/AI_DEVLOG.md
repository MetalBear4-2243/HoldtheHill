...
| 2026-09-23 | nilly-ctrl | Claude Code (Claude Opus 5) | Diagnosed ~26 TextMesh Pro files reporting as modified on every fresh clone — the LFS rules match them by extension but they were committed as plain Git blobs, so the pointer never matches the stored binary; exempted the TMP folder in `.gitattributes` | Team picked this over migrating them into LFS (pure churn — package assets never change) or `git lfs migrate` (rewrites SHAs, breaks open branches and PRs) | — verified by touching all 26 files and forcing an index refresh; working tree stays clean |
